...
$go = clear_string($_GET["go"]);
    ...
    switch($sorting){
        case 'price-asc';
        $sorting='price ASC';
        $sort_name='От дешовых к дорогим';break;
        
        case 'price-desc';
        $sorting='price DESC';
        $sort_name='От дорогих к дешовым';break;
        
        case 'popular';
        $sorting='count DESC';
        $sort_name='Популярное';break;
    
        case 'news';
        $sorting='datetime DESC';
        $sort_name='Новинки';break;
        
        case 'brand';
        $sorting='brand';
        $sort_name='От А до Я';break;
        
        default:
        $sorting='products_id DESC';
        $sort_name='Нет сортировки';break;
    }
...
<title>Интернет магазин футбольной амуниции</title>
</head>
<body>
<div id="block-body">
...
<?php
    ...
    $count = mysql_query("SELECT COUNT(*) FROM table_products $query_aystopper $querycat",$link);
...
<ul id="sorting-list">
<li><a href="view_aystopper.php?go=<?php echo $go;?>&sort=price-asc">От дешовых к дорогим</a></li>
<li><a href="view_aystopper.php?go=<?php echo $go;?>&sort=price-desc">От дорогих к дешовым</a></li>
...
<?php
 ...
 echo '</ul>';  
   } else{ echo '<p>Товары отсутствуют.</p>';}
}else {  echo '<p>Данная категория не найдена</p>';}
